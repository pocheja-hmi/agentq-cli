...
import { resolveTarget, type ResolvedTarget } from '../lib/tier-resolver.js';
...
/** Common shape every subcommand needs. */
interface CommonArgs {
  'project-dir'?: string;
  tier?: string;
}

interface SubArgsBase extends CommonArgs {}
interface ShowArgs   extends SubArgsBase {}
interface DiffArgs   extends SubArgsBase { json: boolean }
interface PlanArgs   extends SubArgsBase { out?: string }
interface ApplyArgs  extends SubArgsBase { plan: string }
interface ImportArgs extends SubArgsBase { 'resource-name': string; 'with-kb': boolean }
interface RmArgs     extends SubArgsBase { yes: boolean }

// ─── Helpers shared across subcommands ──────────────────────────────────────

async function resolveContext(argv: CommonArgs) {
  ...
  if (!target.tier && !cfg.gitops?.enabled) {
    throw new AgentqError(
      '`agentq state` requires GitOps mode.',
      'Enable gitops in agentq.config.yaml (set gitops.enabled: true and populate tiers.*) or use the legacy `agentq deploy` flow.',
    );
  }
  return { root, cfg, pp, target };
}

async function buildPyCommonArgs(
  ...
  if (target.tier) args.push('--tier', target.tier);
...
async function handleShow(argv: ShowArgs): Promise<void> {
...
async function handleDiff(argv: DiffArgs): Promise<void> {
  const { cfg, pp, target } = await resolveContext(argv);
...
async function handlePlan(argv: PlanArgs): Promise<void> {
  const { cfg, pp, target } = await resolveContext(argv);
...
async function handleApply(argv: ApplyArgs): Promise<void> {
  const { cfg, pp, target } = await resolveContext(argv);
...
async function handleImport(argv: ImportArgs): Promise<void> {
  ...
  if (argv['with-kb']) args.push('--with-kb');

  await runPython(pp, 'agentq_runtime.state', ['import', ...args]);
  await uploadStateIfChanged(target, localPath, existed ? generation : null);
}

async function handleRm(argv: RmArgs): Promise<void> {
  const { target } = await resolveContext(argv);
  if (!argv.yes) {
    throw new AgentqError(
      `Refusing to delete state at ${stateUri(target)} without --yes.`,
      'This does NOT destroy the deployed engine; it only removes the state file. To destroy: `agentq destroy --tier <t> <resource>`.',
    );
  }
...
export const stateCommand: CommandModule = {
  ...
  builder: (y: Argv) => y
    .command<ShowArgs>(
      'show',
      'Pretty-print the state file for a tier.',
      (yy) => yy
        .option('project-dir', { type: 'string' })
        .option('tier',        { type: 'string', choices: TIERS as readonly string[] }) as Argv<ShowArgs>,
      handleShow,
    )
    .command<DiffArgs>(
      'diff',
      'Compare local source with remote state. Exits 3 when drift is detected.',
      (yy) => yy
        .option('project-dir', { type: 'string' })
        .option('tier',        { type: 'string', choices: TIERS as readonly string[] })
        .option('json',        { type: 'boolean', default: false }) as Argv<DiffArgs>,
      handleDiff,
    )
    .command<PlanArgs>(
      'plan',
      'Compute and persist a plan JSON for the next apply.',
      (yy) => yy
        .option('project-dir', { type: 'string' })
        .option('tier',        { type: 'string', choices: TIERS as readonly string[] })
        .option('out',         { type: 'string', describe: 'Plan output path. Default: .agentq/plans/<tier>-<sha8>.json' }) as Argv<PlanArgs>,
      handlePlan,
    )
    .command<ApplyArgs>(
      'apply',
      'Execute a plan JSON. Aborts if state has changed since the plan was computed.',
      (yy) => yy
        .option('project-dir', { type: 'string' })
        .option('tier',        { type: 'string', choices: TIERS as readonly string[] })
        .option('plan',        { type: 'string', demandOption: true, describe: 'Path to plan.json.' }) as Argv<ApplyArgs>,
      handleApply,
    )
    .command<ImportArgs>(
      'import',
      'Stamp an existing Reasoning Engine into the tier state file (migration path).',
      (yy) => yy
        .option('project-dir',   { type: 'string' })
        .option('tier',          { type: 'string', choices: TIERS as readonly string[] })
        .option('resource-name', { type: 'string', demandOption: true, describe: 'Full resource name of the existing engine.' })
        .option('with-kb',       { type: 'boolean', default: true, describe: 'Also seed the state with current local KB files (default true).' }) as Argv<ImportArgs>,
      handleImport,
    )
    .command<RmArgs>(
      'rm',
      'Delete the tier state file. Does NOT destroy the engine.',
      (yy) => yy
        .option('project-dir', { type: 'string' })
        .option('tier',        { type: 'string', choices: TIERS as readonly string[] })
        .option('yes',         { type: 'boolean', alias: 'y', default: false }) as Argv<RmArgs>,
      handleRm,
    )
    .demandCommand(1)
    .strict(),
  handler: () => { /* yargs dispatches to sub-handlers */ },
};
