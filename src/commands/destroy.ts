...
export const destroyCommand: CommandModule<{}, Args> = {
  ...
  builder: (y: Argv) =>
    y.positional('resource', { type: 'string', describe: 'Full resource name (projects/.../reasoningEngines/...).' })
     ...
     .option('tier',        { type: 'string', choices: TIERS as readonly string[], describe: 'Tier the resource belongs to (state file will be deleted). Inferred from resource_name when omitted.' })
  ...
  handler: async (argv) => {
    ...
    let tierMatch: string | null = argv.tier ?? null;
    if (!tierMatch && cfg.gitops?.enabled) {
      // Search each tier's state for a matching resource_name.
      for (const t of Object.keys(cfg.tiers ?? {}) as Array<keyof typeof cfg.tiers>) {
        const target = resolveTarget(cfg, t as string);
        try {
          const result = await stateRead(target);
          if (result?.state.engine?.resource_name === argv.resource) {
            tierMatch = t as string;
            break;
          }
        } catch {
    ...
    if (willClearStateFile) {
      try {
        ...
        log.warn(`Could not delete state file: ${(e as Error).message}`);
      }
    }
    if (willClearLegacyConfig) log.info('Cleared resource_name from agentq.config.yaml.');
    if (argv.purge)            log.info('Purged staging artifacts.');
  },
};
