...
export const doctorCommand: CommandModule<{}, Args> = {
  command: 'doctor',
  ...
  handler: async (argv) => {
    ...
    try {
      ...
      checks.push({ name: 'gcloud installed', status: 'fail', detail: (err as Error).message });
    }
    ...
    try {
      ...
      checks.push({ name: 'system Python', status: 'fail', detail: (err as Error).message });
    }
