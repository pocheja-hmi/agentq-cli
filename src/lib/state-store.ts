...
import { StateSchema, type AgentqState } from './state-schema.js';
import type { ResolvedTarget } from './tier-resolver.js';

export class ConcurrentDeployError extends AgentqError {
  constructor(stateUri: string, expectedGen: number | null, actualGen: number | null) {
    super(
      `Concurrent deploy detected on ${stateUri}.`,
      `The state file was modified between plan and apply. Re-run \`agentq state plan\` and \`agentq state apply\` from scratch. (expected generation ${expectedGen}, actual ${actualGen})`,
    );
  }
}

export class StateNotFoundError extends AgentqError {
  constructor(stateUri: string) {
    super(
      `No state file at ${stateUri}.`,
      'This tier has never been deployed. Run `agentq deploy --tier <t>` (first deploy) or `agentq state import --tier <t> --resource-name ...` to import an existing engine.',
    );
  }
}
...
export interface ReadResult {
  state: AgentqState;
  /** GCS object generation. Pass this to write() to enforce optimistic concurrency. */
  generation: number;
}
...
function parseStateUri(target: ResolvedTarget): { bucket: string; objectName: string } {
  if (!target.state_bucket || !target.state_path) {
    throw new AgentqError(
      'state-store requires a tier target (state_bucket + state_path).',
      'Pass --tier on the command, or enable gitops in agentq.config.yaml.',
    );
  }
...
export function stateUri(target: ResolvedTarget): string {
 ...
 */
export async function read(target: ResolvedTarget): Promise<ReadResult | null> {
  const { bucket, objectName } = parseStateUri(target);
  ...
  try {
    ...
    metadata = (await blob.getMetadata())[0] as { generation?: string | number };
  } catch (err) {
    const code = (err as { code?: number }).code;
    if (code === 404) return null;
    throw new AgentqError(
      `Failed to read ${stateUri(target)}: ${(err as Error).message}`,
      'Check that the state bucket exists and the deployer SA has roles/storage.objectViewer on it.',
    );
  }
  ...
  try {
    ...
    throw new AgentqError(`Could not parse state YAML at ${stateUri(target)}: ${(e as Error).message}`);
  }
...
export async function write(
  ...
  try {
    const options: Parameters<typeof blob.save>[1] = {
      contentType: 'application/yaml',
      metadata: { cacheControl: 'no-cache, no-store, max-age=0' },
    };
    if (ifGenerationMatch != null) {
      (options as { preconditionOpts?: { ifGenerationMatch: number } }).preconditionOpts =
        { ifGenerationMatch };
    }
    await blob.save(body, options);
    const meta = (await blob.getMetadata())[0] as { generation?: string | number };
    ...
    throw new AgentqError(
      `Failed to write ${stateUri(target)}: ${(err as Error).message}`,
      'Check that the deployer SA has roles/storage.objectAdmin on the state bucket.',
    );
  }
}

/** Delete the state object. Used by `agentq destroy` and `agentq state rm`. */
export async function remove(target: ResolvedTarget): Promise<boolean> {
  const { bucket, objectName } = parseStateUri(target);
  const blob = client().bucket(bucket).file(objectName);
  try {
    const [exists] = await blob.exists();
    ...
    throw new AgentqError(
      `Failed to delete ${stateUri(target)}: ${(err as Error).message}`,
    );
  }
}

// FOR TESTING ONLY — let tests inject a fake Storage client.
export function _setStorageClient(c: Storage | null): void {
  _client = c;
}
