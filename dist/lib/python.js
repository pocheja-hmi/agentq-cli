...
export async function findSystemPython() {
    ...
    throw new AgentqError('No Python interpreter found on PATH.', 'Install Python 3.10+ from https://www.python.org/ or via your package manager.');
}
function venvPython(projectPaths) {
 ...
 */
async function runQuiet(spinnerText, cmd, args, opts) {
    if (opts.verbose) {
        log.info(spinnerText);
        await execa(cmd, args, { stdio: 'inherit', cwd: opts.cwd });
        return;
    }
    const spinner = ora({ text: spinnerText, color: 'blue' }).start();
    const buffer = [];
    ...
    throw new AgentqError(`${cmd} failed with exit code ${result.exitCode ?? 'unknown'}.`, 'Re-run with --verbose to see full output.');
}
export async function ensureVenv(projectPaths, opts = {}) {
