...
function buildLegacyTarget(cfg) {
    return {
        tier: null,
...
function legacyDatastoreResource(cfg) {
    const kb = cfg.knowledge_base;
    if (kb.provider !== 'gemini-enterprise-search' || !kb.datastore_id)
        return null;
...
function buildTierTarget(cfg, tier) {
    if (!cfg.tiers || !cfg.tiers[tier]) {
        throw new AgentqError(`tiers.${tier} is not defined in agentq.config.yaml.`, `Define it under tiers.* or pick a tier that exists: ${Object.keys(cfg.tiers ?? {}).sort().join(', ') || '(none)'}`);
    }
...
function isTier(value) {
    return value != null && TIERS.includes(value);
}
/**
 ...
 */
export function resolveTarget(cfg, tierFlag) {
    if (tierFlag != null) {
        if (!isTier(tierFlag)) {
            throw new AgentqError(`Unknown tier: ${JSON.stringify(tierFlag)}. Must be one of: ${TIERS.join(', ')}.`);
        }
...
export function stateObjectPath(target) {
    if (!target.state_bucket || !target.state_path) {
        throw new AgentqError('State path is only defined for tier targets.', 'Use --tier to operate in tier mode, or update agentq.config.yaml with gitops.enabled=true and tiers.*.');
    }
