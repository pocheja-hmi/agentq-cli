...
export const deployCommand = {
    ...
    builder: (y) => y.option('project-dir', { type: 'string', describe: 'Project root (defaults to walk-up from cwd).' })
        .option('tier', { type: 'string', choices: TIERS, describe: 'Deploy to a specific tier (GitOps mode).' })
    ...
    handler: async (argv) => {
        ...
        if (target.tier) {
            ...
            for (const flag of ['update', 'recreate', 'resource-name']) {
                if (argv[flag]) {
                    throw new AgentqError(`--${flag} is not supported in tier mode (--tier ${target.tier}).`, 'Use `agentq state plan` + `agentq state apply` for finer control, or `agentq destroy --tier <t>` + a fresh `agentq deploy --tier <t>` to recreate.');
                }
            }
        ...
        if (argv.update && !hasPersisted && !argv['resource-name']) {
            throw new AgentqError('--update requested but no resource_name is persisted in agentq.config.yaml.', 'Either drop the flag (a first deploy auto-creates) or pass --resource-name explicitly.');
        }
        const args = [path.join(pp.root, 'agentq.config.yaml')];
        args.push(mode === 'update' ? '--update' : '--create');
        if (argv['resource-name'])
        ...
        if (mode === 'create' && hasPersisted) {
            log.warn(`Previous resource ${cfg.deployment.resource_name} is NOT being deleted. ` +
        ...
        try {
            ...
            if (code === 3) {
                throw new AgentqError(`Persisted resource ${cfg.deployment.resource_name} no longer exists.`, 'Run `agentq deploy --recreate` to deploy a fresh Reasoning Engine. The new resource_name will replace the stale one in agentq.config.yaml automatically.');
            }
            throw err;
        }
    },
};
async function deployTierMode(cfg, pp, target, root) {
