...
function expandRef(ref, project) {
...
export const verifySecretsCommand = {
    command: 'verify-secrets',
    describe: 'Verify each *_SECRET_REF is readable by the tier runtime SA (CI + pre-deploy preflight).',
    builder: (y) => y.option('tier', { type: 'string', describe: 'Tier to verify (dev|staging|prod). Defaults to the gitops default tier.' })
        .option('project-dir', { type: 'string' })
        .option('impersonate', { type: 'boolean', default: true, describe: 'Test as the runtime SA (impersonation). --no-impersonate tests as the caller.' }),
    handler: async (argv) => {
        const root = argv['project-dir'] ? path.resolve(argv['project-dir']) : (await findProjectRoot());
        if (!root)
            throw new AgentqError('Not in an AgentQ project (no agentq.config.yaml found).');
        ...
        const refs = Object.entries((cfg.runtime.env_vars ?? {}))
        ...
        if (argv.impersonate && !runtimeSa) {
            throw new AgentqError(`No runtime_service_account for tier '${target.tier ?? 'default'}' — cannot verify secret access.`, 'Set tiers.<t>.runtime_service_account, or run with --no-impersonate.');
        }
        log.banner(`verify-secrets — tier ${target.tier ?? '(legacy)'} → ${project}`);
        if (argv.impersonate)
            log.info(`Testing as runtime SA: ${runtimeSa}`);
        const failures = [];
        for (const [key, rawRef] of refs) {
            ...
            if (argv.impersonate && runtimeSa)
                args.push(`--impersonate-service-account=${runtimeSa}`);
            try {
                await gcloud(args);
        ...
        if (failures.length > 0) {
            for (const f of failures)
                log.error(f);
            throw new AgentqError(`${failures.length} secret(s) not readable by the ${target.tier ?? 'target'} runtime SA.`, `Provision per GCP: agentq setup-cicd --gcp-project ${project} ... --secret <name>, then add the value. ` +
...
//# sourceMappingURL=verify-secrets.js.map