...
export const kbCommand = {
    ...
    builder: (y) => y.positional('subcommand', { type: 'string', describe: 'KB subcommand (omit to see help).' })
        .positional('args', { type: 'string', array: true, describe: 'Arguments forwarded to the subcommand.' })
        .option('project-dir', { type: 'string' })
        .option('tier', { type: 'string', choices: TIERS, describe: 'Operate on a specific tier (defaults to gitops.default_tier or legacy).' })
        .option('allow-prod-kb-mutation', { type: 'boolean', default: false, describe: 'Override the tier policy that disallows direct KB mutation on staging/prod tiers.' }),
    handler: async (argv) => {
        ...
        if (providerId === 'none') {
            throw new AgentqError(target.tier
                ? `Tier ${target.tier} has no kb.datastore_id configured.`
                : 'This project was scaffolded without a knowledge base.', target.tier
        ...
        if (!provider) {
            ...
            throw new AgentqError(`Unknown KB provider: ${providerId}`, `Known providers: ${known}`);
        }
        if (!argv.subcommand) {
        ...
        if (MUTATING_SUBS.has(argv.subcommand)
            ...
            throw new AgentqError(`Tier '${target.tier ?? '(legacy)'}' disallows direct KB mutation.`, 'Pass --allow-prod-kb-mutation to override (only the GitOps workflow should do this).' +
        ...
        if (!handler) {
            throw new AgentqError(`Unknown subcommand: ${argv.subcommand}`, `Available: ${Object.keys(subs).join(', ')}`);
        }
